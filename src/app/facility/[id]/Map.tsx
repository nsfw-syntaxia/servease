...
const Map = ({ address, facilityName }: MapProps) => {
  ...
  useEffect(() => {
    ...
    const geocodeAddress = async () => {
      try {
        setLoading(true);
        const results = await maptilersdk.geocoding.forward(address, {
          limit: 1,
        });
...
const styles = {
  mapLoading:
    "flex items-center justify-center h-full w-full bg-gray-200 text-gray-600",
  mapError:
    "flex items-center justify-center h-full w-full bg-red-100 text-red-700 p-4",
};
