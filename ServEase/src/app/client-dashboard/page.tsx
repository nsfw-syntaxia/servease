...
export default async function ClientDashboardPage() {
  ...
  try {
    ...
    const { data: rawAppointments } = await supabase
      .from('appointments')
      .select('id, date, time, status, address, provider_id, provider:provider_id(business_name, picture_url)')
    ...
    const { data: rawFeaturedServices, error: rpcError } = await supabase
      .rpc('get_random_featured_services_with_provider');
      ...
      rawFeaturedServices?.map(async (service, index) => {
        ...
        if (providerProfile?.picture_url) {
          if (providerProfile.picture_url.startsWith("http")) {
            ...
            const { data } = supabase.storage.from('avatars').getPublicUrl(providerProfile.picture_url);
        ...
        if (providerProfile?.facility_image_url) {
          if (providerProfile.facility_image_url.startsWith("http")) {
            ...
            const { data } = supabase.storage.from('documents').getPublicUrl(providerProfile.facility_image_url);
